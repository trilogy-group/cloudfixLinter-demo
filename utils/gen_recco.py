...
def generate_dynamodb_reco(region, account, resource_id, resource_name):
    return {
		"id": str(uuid.uuid4()),
		"customerId": 10002,
		"accountId": account,
...
def generate_ec2_low_risk_righ_size_recco(region, account, resource_id, resource_name):
    return {
        "id": str(uuid.uuid4()),
		"customerId": 10002,
...
def generate_recos(item):
    ...
    if item['type'] == 'aws_dynamodb_table':
        return [generate_dynamodb_reco(region, account_id, item['values']['id'], item['name'])]
